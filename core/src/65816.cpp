    ...
    : SchedulerTask(), m_Membus(membus)
{
    ...
    static const struct {
        ...
        {AddressingMode::AbsoluteJMPIndirectIndexedX,
         &Cpu65816::handleAbsoluteJMPIndirectIndexedX},
        ...
        {AddressingMode::AbsoluteIndirectLong,
         &Cpu65816::handleAbsoluteIndirectLong},
        {AddressingMode::AbsoluteLongIndexedX,
         &Cpu65816::handleAbsoluteLongIndexedX},
        ...
        {AddressingMode::DpIndirectIndexedX,
         &Cpu65816::handleDpIndirectIndexedX},
        {AddressingMode::DpIndexedIndirectY,
         &Cpu65816::handleDpIndexedIndirectY},
        {AddressingMode::DpIndirectLong, &Cpu65816::handleDpIndirectLong},
        {AddressingMode::DpIndirectLongIndexedY,
         &Cpu65816::handleDpIndirectLongIndexedY},
        ...
        {AddressingMode::StackRelativeIndirectIndexedY,
         &Cpu65816::handleStackRelativeIndirectIndexedY},
...
int Cpu65816::run()
{
    ...
    if (!opcodeDesc.m_Name) {
        ...
        LOGC(
            TAG,
            "Unknown opcode 0x%02X (Address %06X)",
            opcode,
            m_CurrentOpcodePC);
        assert(false);
    }
    ...
    // Load data
    uint32_t data = 0;
    const auto addressingModeHandler =
        m_AddressingModes[enumToInt(opcodeDesc.m_AddressingMode)];
...
void Cpu65816::logInstruction(const char* format, Args... args)
{
    ...
    auto logBuilder = [opcodePC = m_CurrentOpcodePC,
                       regs = m_Registers,
                       format,
                       argsTuple]() -> std::string {
        char opcodeLog[32];
...
void Cpu65816::handleADC(uint32_t address, int* cycles)
{
    ...
    if (accumulatorSize) {
        ...
        uint32_t result =
            (m_Registers.A & 0xFF) + data + getBit(m_Registers.P, kPRegister_C);

        // V flag
        uint8_t data8 = data;
        if ((m_Registers.A & 0x80) == (data8 & 0x80) &&
            (m_Registers.A & 0x80) != (result & 0x80)) {
        ...
        uint32_t result =
            m_Registers.A + data + getBit(m_Registers.P, kPRegister_C);

        // V flag
        uint16_t data16 = data;
        if (~(m_Registers.A ^ data16) &
            (data16 ^ static_cast<uint16_t>(result)) & 0x8000) {
            m_Registers.P = setBit(m_Registers.P, kPRegister_V);
        } else {
            m_Registers.P = clearBit(m_Registers.P, kPRegister_V);
        }
...
void Cpu65816::handleANDImmediate(uint32_t data, int* cycles)
{
    ...
    // 0: 16 bits, 1: 8 bits
    if (accumulatorSize) {
        m_Registers.A = (m_Registers.A & 0xFF00) |
                        ((m_Registers.A & 0xFF) & data);
...
void Cpu65816::handleJSL(uint32_t data, int* cycles)
{
    ...
    m_Membus->writeU16(
        m_Registers.S - 1, (m_Registers.PC & 0xFFFF) - 1, cycles);
    m_Registers.S -= 2;
...
void Cpu65816::handlePLA(uint32_t data, int* cycles)
{
    ...
    if (accumulatorSize) {
        m_Registers.A = (m_Registers.A & 0xFF00) |
                        m_Membus->readU8(m_Registers.S + 1, cycles);
        m_Registers.S++;
        setNZFlags(m_Registers.A & 0xFF, 0x80);
    } else {
...
void Cpu65816::handleSBCImmediate(uint32_t rawData, int* cycles)
{
    ...
    if (accumulatorSize) {
        ...
        if (~((m_Registers.A & 0xFF) ^ data) &
            ((m_Registers.A & 0xFF) ^ result) & 0x80) {
            m_Registers.P = setBit(m_Registers.P, kPRegister_V);
        } else {
            m_Registers.P = clearBit(m_Registers.P, kPRegister_V);
        }
    ...
    int* cycles)
{
    ...
    if (accumulatorSize) {
        *data =
            m_Membus->readU8((m_Registers.PB << 16) | m_Registers.PC, cycles);
        m_Registers.PC += 1;

        logInstruction("%s #$%02X", opcodeDesc.m_Name, *data);
    } else {
        *data =
            m_Membus->readU16((m_Registers.PB << 16) | m_Registers.PC, cycles);
        m_Registers.PC += 2;
    ...
    int* cycles)
{
    ...
    address = (m_Registers.PB << 16) |
              m_Membus->readU16(address + m_Registers.X, cycles);
    ...
    int* cycles)
{
    ...
    *data = m_Membus->readU24(rawData, cycles);
...
{
    uint32_t rawData =
        m_Membus->readU24((m_Registers.PB << 16) | m_Registers.PC, cycles);
    m_Registers.PC += 3;
    ...
    int* cycles)
{
    ...
    uint32_t address = m_Registers.D + rawData + m_Registers.X;
    address = ((m_Registers.DB << 16) | m_Membus->readU16(address, cycles));
    ...
    int* cycles)
{
    ...
    address = (m_Registers.DB << 16) |
              (m_Membus->readU16(address, cycles) + m_Registers.Y);
    ...
    int* cycles)
{
    ...
    *data = m_Membus->readU16(
        (m_Registers.PB << 16) | (m_Registers.PC + 1), cycles);

    logInstruction(
        "%s $%02X, $%02X", opcodeDesc.m_Name, *data >> 8, *data & 0xFF);
}
...
void Cpu65816::addCyclesIndexCross(
    int* cycles,
    uint32_t addr,
    uint32_t shiftedAddr)
{
    auto indexSize = getBit(m_Registers.P, kPRegister_X);
