...
void SnesImpl::DurationTool::reset()
{
    total_duration = SnesImpl::Clock::duration::zero();
}

void SnesImpl::DurationTool::begin()
{
    if (!kLogTimings) {
        return;
    }
...
void SnesImpl::DurationTool::end()
{
    if (!kLogTimings) {
        return;
    }
...
std::shared_ptr<Snes> Snes::create()
{
...
int SnesImpl::removeRenderer(const std::shared_ptr<Renderer>& renderer)
{
...
int SnesImpl::plugCartidge(const char* path)
{
    uint8_t romRamInfo;
...
int SnesImpl::scoreHeader(uint32_t address)
{
    int score = 0;
...
int SnesImpl::start()
{
    ...
    auto renderCb = [this](const Color& c) {
        for (const auto& renderer : m_RendererList) {
...
SnesConfig SnesImpl::getConfig()
{
    SnesConfig config;

    config.displayWidth = kPpuDisplayWidth;
    config.displayHeight = kPpuDisplayHeight;
    config.displayRate = 60;

    config.audioChannels = 2;
    config.audioSampleSize = Apu::kSampleSize;
    config.audioSampleRate = Apu::kSampleRate;

    return config;
}

int SnesImpl::renderSingleFrame(bool renderPpu)
{
...
void SnesImpl::resumeTask(SchedulerTask* task, int cycles)
{
    task->setNextRunCycle(m_MasterClock + cycles);
}

void SnesImpl::setHVIRQ_Flag(bool v)
{
    m_HVIRQ_Flag = v;
    m_Cpu->setIRQ(v);
}
...
void SnesImpl::saveState(const std::string& path)
{
...
void SnesImpl::loadState(const std::string& path)
{
...
uint8_t SnesImpl::readU8(uint32_t addr)
{
    switch (addr) {
    case kRegRDNMI: {
...
void SnesImpl::writeU8(uint32_t addr, uint8_t value)
{
    switch (addr) {
    case kRegNmitimen: {
        // H/V IRQ
